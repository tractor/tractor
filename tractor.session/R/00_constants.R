...
.DefaultSessionDirectories <- list(diffusion="diffusion", camino="camino", fdt="fdt", bedpost="fdt.bedpostX", probtrack="fdt.track", transforms="transforms", structural="structural", freesurfer="freesurfer")

.PrimarySessionDirectories <- c("diffusion", "structural")
...
.DefaultSessionMap <- list(
    diffusion=list(rawdata="rawdata", data="data", refb0="refb0", mask="mask", maskedb0="maskedb0", s0="dti_S0", fa="dti_FA", md="dti_MD", eigenvalue="dti_eigval%", eigenvector="dti_eigvec%", axialdiff="dti_eigval1", radialdiff="dti_radial", sse="dti_SSE", parcellation="parcellation"),
    ...
    freesurfer=list(reft1=file.path("mri","rawavg"), normT1=file.path("mri","T1"), "desikan-killiany"=file.path("mri","aparc+aseg"), destrieux=file.path("mri","aparc.a2009s+aseg")))
...
.RegistrationTargets <- list(mni="brain", diffusion="maskedb0", structural="reft1", freesurfer="normT1")
