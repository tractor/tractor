...
# Graph 3: Undirected, weighted, diagonal filled
vector <- sample(c(runif(20,0,2), rep(0,25)))
...
test_that("Setter functions work",{
    ...
    vector <- sample(c(runif(25,0,2), rep(0,20)))
    m_new <- createSymMatrix(vector, 10)
    diag(m_new) <- runif(10,2,4)
...
test_that("Edge and vertex tests", {
    ...
    t_new_ass_m[lower.tri(t_new_ass_m)] = 0
...
test_that("Edge Normalisation", {
    ...
    ass_m <- t_graph$getAssociationMatrix()
    colwise_count <- which(ass_m[upper.tri(ass_m, diag=TRUE)]>0)
    weights <- ass_m[upper.tri(ass_m, diag=TRUE)]
    non_zero_weights <- weights[colwise_count]
    expect_identical(t_norm_weights, non_zero_weights)
})
...
test_that("Weights detected correctly", {
    ...
    colwise_count <- which(ass_m[upper.tri(ass_m, diag=TRUE)]>0)
    weights <- ass_m[upper.tri(ass_m, diag=TRUE)]
    t_control_weights <- weights[colwise_count]
