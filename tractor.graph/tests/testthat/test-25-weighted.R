...
test_that("Edge and vertex tests", {
    ...
    # Edge(s)
    expect_equivalent(t_graph$getEdge(1), ends(i_graph, 1)[1,]) 
    t_association_matrix[lower.tri(t_association_matrix)] = 0
    expect_equivalent(t_graph$getEdges(), which(t_association_matrix>0, arr.ind = TRUE))
...
test_that("Edge Normalisation", {
    ...
    tmp <- cbind(t_norm_weights,m[upper.tri(m)])
