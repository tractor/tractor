...
void RCallbackDataSink::put (const Streamline &data)
{
    ...
    const ImageSpace::PointType pointType = data.getPointType();
    
    for (size_t i=0; i<points.size(); i++)
    {
        pointsR(i,0) = points[i][0] + (pointType == ImageSpace::VoxelPointType ? 1.0 : 0.0);
        pointsR(i,1) = points[i][1] + (pointType == ImageSpace::VoxelPointType ? 1.0 : 0.0);
        pointsR(i,2) = points[i][2] + (pointType == ImageSpace::VoxelPointType ? 1.0 : 0.0);
    }
    
    const std::string unit = (pointType == ImageSpace::VoxelPointType ? "vox" : "mm");
