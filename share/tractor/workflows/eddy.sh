...
set -x
${FURROW} ${TRACTOR_COMMAND} --imain=@diffusion/rawdata --mask=@diffusion/mask --acqp=@fdt/acqparams.txt --index=@fdt/index.txt --bvecs=@fdt/bvecs --bvals=@fdt/bvals --out=@fdt/data --data_is_shelled "$topup_arg" >"$fdt_dir/eddy.log"
${TRACTOR} image mv @fdt/data @diffusion/data
${TRACTOR} image link @diffusion/data @fdt/data
