...
#@desc Track the interconnections between target regions, which must have been delineated in a parcellation (typically via "parcellate" or "freesurf"). Target regions may be specified individually or by type, using names from the parcellation's lookup table. Seed regions are specified similarly, or can be left unspecified in which case the whole brain mask will be used. Seed regions may be morphologically modified by erosion, dilation, or using combined operations to find an inner or outer boundary; and an anisotropy threshold can also be applied. The ParcellationConfidence variable controls the inclusiveness of the transformed parcellation: the closer to 1, the more conservative.
