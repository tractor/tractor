#@desc Print out various graph-theoretical properties of a graph object. The object may be specified as an argument (which takes priority), or through the GraphName option. Any edge weight threshold specified is applied before the calculations. If IgnoreSign:true is given then then edges with weights greater than the threshold or less than its negative will be retained; this will be done by default if the threshold is positive. This script requires the "igraph" R package. Note that further properties of the graph, such as the attributes which are defined for it, may be seen using the "peek" script.
#@args [graph file]
#@nohistory TRUE
...
runExperiment <- function ()
{
    ...
    values <- c(s("#{graph$nVertices()} (#{length(graph$getConnectedVertices())} connected)"),
                ...
                signif(c(globalEfficiency, localEfficiency),3),
                s("#{meanClusteringCoefficient}#{ifelse(graph$isWeighted(),' (unweighted)','')}",signif=3))
