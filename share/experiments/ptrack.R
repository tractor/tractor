...
#@desc Run tractography for connectivity "profiling", generating a table of streamline counts connecting each voxel in the seed region to each target region. For generating tract images, use "rtrack" instead; for regionwise tracking use "mtrack".
...
runExperiment <- function ()
{
    ...
    for (targetFile in targetMaskFiles)
    {
        ...
        if (targetMasksInStandardSpace)
            targetMask <- transformImageToSpace(targetMask, session, "diffusion", oldSpace="mni")
